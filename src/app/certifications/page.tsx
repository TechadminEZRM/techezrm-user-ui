  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-start gap-6 md:gap-12 w-full bg-white p-6 rounded-[46px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-all hover:shadow-[0_4px_16px_rgba(0,0,0,0.12)] hover:-translate-y-0.5">
      ...
      <div className="flex-1 text-center md:text-left min-w-0">
        <h3 className="font-semibold text-brand text-[1.5rem] md:text-[1.75rem] mb-2 tracking-tight">{certificate.title}</h3>
        <p className="font-medium text-dim text-sm md:text-[1.5rem] mb-4 md:mb-5">{certificate.certificationName}</p>
        <p className="text-dim text-sm md:text-base leading-relaxed mb-4 md:mb-5 max-w-full md:max-w-[650px] mx-auto md:mx-0">{certificate.descr}</p>

        <div className="mb-4 md:mb-5 space-y-1">
          <p className="text-xs md:text-sm text-faint"><strong>Issued by:</strong> {certificate.issuedBy}</p>
          <p className="text-xs md:text-sm text-faint"><strong>Issue Date:</strong> {formatDate(certificate.issueDate)}</p>
          <p className={`text-xs md:text-sm font-medium ${expired ? "text-red-600" : "text-faint"}`}>
            <strong>Expiry Date:</strong> {formatDate(certificate.expiryDate)}{expired && " (Expired)"}
          </p>
          <p className="text-xs md:text-sm text-faint"><strong>Certificate ID:</strong> {certificate.certificateId}</p>
        </div>
...
const CertificationCardSkeleton: React.FC = () => (
  <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-12 w-full bg-white p-6 rounded-[46px] shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
    <div className="flex-shrink-0 flex justify-center md:justify-start w-full md:w-auto">
      <div className="w-[min(80vw,300px)] h-[min(80vw,300px)] md:w-[250px] md:h-[250px] bg-wash rounded-lg animate-pulse" />
    </div>
    <div className="flex-1 text-center md:text-left">
      <div className="h-7 bg-wash rounded w-3/5 mb-3 animate-pulse" />
      <div className="h-5 bg-wash rounded w-4/5 mb-5 animate-pulse" />
      <div className="h-4 bg-wash rounded w-full mb-2 animate-pulse" />
      <div className="h-4 bg-wash rounded w-[90%] mb-2 animate-pulse" />
      <div className="h-4 bg-wash rounded w-[70%] mb-5 animate-pulse" />
      <div className="h-3.5 bg-wash rounded w-2/5 mb-1 animate-pulse" />
      <div className="h-3.5 bg-wash rounded w-1/2 mb-1 animate-pulse" />
      <div className="h-3.5 bg-wash rounded w-[45%] mb-5 animate-pulse" />
      <div className="h-3.5 bg-wash rounded w-[30%] animate-pulse" />
    </div>
  </div>
);
...
const Certifications: React.FC = () => {
  ...
  return (
    <div className="w-full bg-paper py-6 md:py-0">
      <div className="w-full px-0">
        <div className="bg-paper rounded-lg p-6 md:p-8 min-h-[300px]">
          {/* Heading */}
          <h2 className="font-semibold text-[#1a1a1a] text-[1.75rem] md:text-[2rem] mb-6 md:mb-8 ml-8 tracking-tight">
            Certifications
            {!isLoading && certificates.length > 0 && (
              <span className="ml-4 text-dim text-base font-normal">({certificates.length} total)</span>
            )}
          </h2>
          ...
          {!isLoading && !isError && certificates.length === 0 && (
            <div className="text-center py-8 mx-8">
              <p className="text-lg font-medium text-dim mb-1">No certifications found</p>
              <p className="text-sm text-faint">Certifications will appear here once they are available.</p>
            </div>
          )}

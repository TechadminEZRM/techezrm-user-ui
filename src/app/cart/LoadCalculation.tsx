...
const LoadCalculation: React.FC = () => {
  ...
  return (
    <div className="mt-8 w-full">
      <p className="text-lg font-semibold text-[#333] mb-5">Load & Stuffing Calculation</p>
        ...
        <div className="flex items-center bg-[rgba(251,251,251,1)] min-h-[56px]">
          {tabs.map((tab, index) => (
            <div
              ...
              className={`flex-1 flex items-center justify-center h-14 cursor-pointer text-[13px] font-semibold transition-all relative hover:text-[#F9A922] ${index < 2 ? "border-r-[3px] border-[#f0f0f0]" : ""} ${activeTab === index ? "text-[#F9A922] border-b-2 border-b-[#F9A922]" : "text-[rgba(90,96,127,0.77)] border-b-2 border-b-transparent"}`}
            >
              {tab}
            </div>
          ))}
          <button className="m-2 border border-transparent rounded w-9 h-9 flex items-center justify-center bg-white hover:bg-[#f5f5f5]">
            <Settings className="w-[18px] h-[18px] text-[#999]" />
          </button>
        </div>
      ...
      {isTruckModalOpen && (
        <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-black/50">
          <div className="w-[95%] sm:w-[85%] md:w-[75%] lg:w-[1000px] max-h-[90vh] bg-white rounded-[8px] shadow-[0_8px_30px_rgba(0,0,0,0.2)] overflow-hidden relative flex flex-col">
            {/* Modal Header */}
            <div className="bg-[#fafafa] px-6 py-4 border-b border-[#e5e5e5] flex items-center justify-between">
              <div className="flex gap-8">
                <span className="flex items-center gap-2 text-[#667080] text-base font-medium cursor-pointer">🏛️ Container</span>
                <span className="flex items-center gap-2 text-[#1890ff] text-base font-semibold cursor-pointer border-b-[3px] border-[#1890ff] pb-2">🚚 Truck</span>
              </div>
              <button onClick={handleCloseTruckModal} className="w-8 h-8 flex items-center justify-center hover:bg-[#f0f0f0] rounded transition-colors">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {trucks.map((truck) => (
                  <div
                    ...
                    className={`cursor-pointer rounded-[8px] p-4 text-center flex flex-col items-center gap-3 min-h-[200px] bg-white transition-all hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)] ${selectedTruck?.id === truck.id ? "border-[3px] border-[#1890ff]" : "border border-[#e1e6ed] hover:border-[#1890ff]"}`}
                  >
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="w-[120px] h-[80px] bg-[#f8f9fa] rounded flex items-center justify-center text-[40px]">🚛</div>
                    </div>
            ...
            <div className="px-6 py-4 bg-[#fafafa] border-t border-[#e5e5e5] flex justify-end gap-3">
              <button onClick={handleCloseTruckModal} className="font-semibold text-sm px-8 py-2 rounded-[8px] border border-[#e1e6ed] text-[#5992ff] hover:border-[#5992ff] hover:bg-[rgba(89,146,255,0.04)] transition-colors">Cancel</button>
              <button
                onClick={handleTruckSelect}
                disabled={!selectedTruck}
                className="font-semibold text-sm px-8 py-2 rounded-[8px] bg-[#1890ff] hover:bg-[#0c7cd5] disabled:bg-[#d1d5db] disabled:text-[#9ca3af] text-white transition-colors"
              >
                Select
              </button>
            </div>
      ...
      {isContainerModalOpen && (
        <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-[8px] shadow-[0_8px_30px_rgba(0,0,0,0.2)] p-8 min-w-[300px] text-center">
            ...
            <p className="text-[#666] mb-6">Container functionality will be available soon!</p>
            <button onClick={handleCloseContainerModal} className="bg-[#ff8144] hover:bg-[#E8981F] text-white px-8 py-2 rounded text-sm font-semibold transition-colors">Close</button>
          </div>
        </div>
      )}
    </div>

...
const FavouritesSection: React.FC = () => {
  ...
  if (!isAuthenticated || !customer) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-semibold text-body mb-6">Wishlist</h1>
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <p className="text-lg text-dim mb-6">Please login to view your favourites</p>
          <button onClick={() => router.push("/sign_in")} className="bg-brand hover:bg-brand-hover text-white px-8 py-3 text-base font-semibold rounded transition-colors">Login</button>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col items-center justify-center py-16 px-8 text-center min-h-[60vh]">
          <div className="w-40 h-40 mb-8 relative" style={{ animation: "float 3s ease-in-out infinite" }}>
            <div className="w-full h-full rounded-[25px] border-[3px] border-dashed border-brand flex items-center justify-center shadow-[0_12px_40px_rgba(255,107,53,0.15)]" style={{ background: "linear-gradient(135deg, var(--color-brand-light) 0%, var(--color-brand-light) 100%)" }}>
              <Heart className="w-16 h-16 text-brand" style={{ animation: "heartBeat 2s ease-in-out infinite" }} />
            </div>
          </div>
          <h2 className="text-2xl md:text-3xl font-semibold text-body mb-4" style={{ animation: "fadeInUp 1s ease-out 0.5s both" }}>No Favourites Yet</h2>
          <p className="text-dim mb-8 max-w-[500px] text-sm md:text-lg leading-relaxed" style={{ animation: "fadeInUp 1s ease-out 0.8s both" }}>
          ...
          <button
            ...
            className="bg-brand hover:bg-brand-hover text-white px-12 py-4 text-lg font-semibold rounded-[12px] shadow-[0_6px_20px_rgba(255,107,53,0.3)] hover:shadow-[0_8px_30px_rgba(255,107,53,0.6)] hover:-translate-y-0.5 transition-all"
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 md:px-4 py-4 md:py-8">
      <h1 className="text-xl md:text-2xl font-semibold text-body mb-4 md:mb-6">Wishlist ({products.length})</h1>
            ...
            {products.slice(rowIndex * 3, rowIndex * 3 + 3).map((product) => (
              ...
              >
                ...
                <div className="flex-1 flex flex-col p-3 md:p-4">
                  <h3 className="font-semibold text-body text-sm md:text-base mb-1 truncate">{product.title}</h3>
                  <p className="text-dim text-xs md:text-sm mb-2 line-clamp-2">{product.subtitle}</p>
                  <p className="text-brand text-sm md:text-base font-semibold mb-1">
                    ${product.price.toLocaleString()} <small className="text-faint">/kg</small>
                  </p>
                  <p className="text-dim text-xs mb-4">Product Code: {product.productCode}</p>
                  ...
                  <div className="flex justify-between items-center mt-auto gap-2">
                    <button
                      ...
                      className={`p-1.5 rounded-full transition-colors hover:bg-red-50 disabled:opacity-60 ${product.isFavorite ? "text-red-400" : "text-line-light"}`}
                    >
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className={`w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded text-white transition-colors ${product.inStock ? "bg-success hover:bg-[#45a049]" : "bg-line-light cursor-not-allowed"}`}
                      >
                      ...
                      <button
                        ...
                        className={`text-[0.65rem] md:text-[0.7rem] font-semibold uppercase px-3 md:px-4 py-1 md:py-1.5 rounded text-white transition-colors ${product.inStock ? "bg-brand hover:bg-brand-hover" : "bg-line-light cursor-not-allowed"}`}
                      >

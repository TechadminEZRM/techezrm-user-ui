...
const inputClass = "w-full border border-line-light rounded bg-white px-3 py-2 text-sm text-body placeholder:text-faint focus:outline-none focus:border-brand hover:border-line-light transition-colors";
...
const CheckoutForm: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <h1 className="text-xl md:text-2xl font-semibold text-body mb-6">Complete your Order</h1>

      {loading && (
        <div className="flex items-center justify-center py-8 gap-3">
          <Spinner size="sm" className="border-brand border-t-transparent" />
          <span className="text-dim">Loading your information...</span>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          {/* Cart Items */}
          <div className="bg-white rounded-xl border border-line shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-8 mb-8">
            <h2 className="text-heading font-bold text-xl mb-6">Cart Items ({cartItems.length})</h2>
            {cartLoading ? (
              <div className="text-center py-8 flex flex-col items-center gap-2">
                <Spinner size="sm" className="border-brand border-t-transparent" />
                <span className="text-dim">Loading cart items...</span>
              </div>
            ) : cartItems.length > 0 ? (
              <div className="max-h-[400px] overflow-y-auto flex flex-col gap-3">
                {cartItems.map((item, index) => (
                  <div key={index} className="p-4 rounded-[12px] border border-brand/10 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_16px_rgba(255,107,53,0.1)] hover:-translate-y-px hover:border-brand/20 transition-all bg-gradient-to-br from-white to-paper">
                    <div className="flex gap-4 items-center">
                      <div className="w-[60px] h-[60px] rounded-[10px] overflow-hidden flex-shrink-0 border-2 border-brand/10 shadow-[0_2px_8px_rgba(0,0,0,0.1)]">
                        <img src="/product.png" alt={item.productName || "Product"} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-heading text-sm font-semibold mb-0.5 truncate">{item.productName || "Product"}</p>
                        <p className="text-soft text-xs mb-2 font-mono">ID: {item?.product?._id?.slice(-8) || "N/A"}</p>
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-sm font-bold" style={{ background: "linear-gradient(135deg, var(--color-brand) 0%, var(--color-brand-hover) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                            ${(item.productPrice || 0) * item.quantity}
                          </span>
                          <div className="flex items-center gap-1.5">
                            <span className="text-soft text-xs font-medium">Qty:</span>
                            <span className="bg-brand/10 text-brand px-2.5 py-1 rounded-[6px] text-xs font-semibold border border-brand/20 min-w-[32px] text-center">{item.quantity}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <p className="text-soft text-center py-8">No items in cart</p>
            )}
          </div>

          {/* Address Management */}
          <div className="bg-white rounded-xl border border-line shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-heading font-bold text-xl">Shipping Address</h2>
              <button onClick={() => setAddressModalOpen(true)} className="bg-brand hover:bg-brand-hover text-white text-sm font-semibold px-4 py-2 rounded transition-colors">
                Add New Address
              </button>
            </div>
            ...
            {!addressesLoading && addresses.length > 0 && (
              <div className="mb-8">
                <p className="text-soft font-semibold text-sm mb-3">Saved Addresses ({addresses.length})</p>
                  ...
                  {addresses.map((address) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-5 rounded-lg cursor-pointer relative transition-all hover:-translate-y-px ${isSelected ? "border-2 border-brand bg-[rgba(255,107,53,0.08)]" : isDefault ? "border border-line-light bg-[rgba(255,107,53,0.03)]" : "border border-line-light bg-white"} hover:shadow-[0_4px_12px_rgba(255,107,53,0.15)] hover:border-brand`}
                      >
                        {isDefault && <span className="absolute top-2 right-2 text-[0.65rem] font-bold bg-brand text-white px-2 py-0.5 rounded-full">DEFAULT</span>}
                        {isSelected && <div className="absolute top-2 left-2 w-4 h-4 rounded-full bg-brand flex items-center justify-center text-white text-[0.7rem] font-bold">✓</div>}
                        <div className={isDefault ? "pr-16" : ""}>
                          <div className="flex items-center gap-2 mb-3">
                            <AddressTypeIcon type={address.type} />
                            <span className={`text-xs font-bold uppercase tracking-wide ${isSelected ? "text-brand" : "text-body"}`}>{address.type} Address</span>
                          </div>
                          <div className="ml-6">
                            {address.companyName && <p className="text-xs font-semibold text-heading mb-0.5">{address.companyName}</p>}
                            <p className="text-xs font-semibold text-heading mb-0.5">{address.receiverName}</p>
                            <p className="text-xs text-soft leading-snug mb-0.5">{address.street}</p>
                            <p className="text-xs text-soft leading-snug mb-0.5">{address.city}, {address.state} {address.zipCode}</p>
                            <p className="text-xs text-soft leading-snug mb-0.5">{address.country}</p>
                            <p className="text-[0.7rem] text-soft">📞 {address.receiverPhone} • ✉️ {address.receiverEmail}</p>
                          </div>
                        </div>
                      </div>
                    );
          ...
          {/* Personal Details */}
          <div className="bg-white rounded-xl border border-line shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-8 mt-8">
            <p className="text-brand font-semibold mb-6">Personal Details</p>
            <div className="flex flex-wrap gap-4 mb-4">
              <div>
                <label className="text-sm font-medium text-body mb-2 block">First name</label>
              ...
              <div>
                <label className="text-sm font-medium text-body mb-2 block">Last name</label>
            ...
            <div className="flex flex-wrap gap-4">
              <div>
                <label className="text-sm font-medium text-body mb-2 block">Email</label>
              ...
              <div>
                <label className="text-sm font-medium text-body mb-2 block">Phone number</label>
        ...
        <div className="w-full lg:w-[400px] flex-shrink-0">
          <div className="p-8 rounded-xl border border-[#475569] shadow-[0_8px_30px_rgba(0,0,0,0.15)] sticky top-6" style={{ background: "linear-gradient(135deg, var(--color-heading) 0%, var(--color-heading) 100%)" }}>
            <h2 className="text-white font-bold text-xl text-center mb-6">Order Summary</h2>

            {/* Coupon */}
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-3">
                <Tag className="w-[18px] h-[18px] text-success" />
                <span className="text-line font-semibold text-sm">Apply Coupon</span>
              </div>
              <input
                className="w-full bg-white/5 border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/60 focus:outline-none focus:border-brand transition-colors"
                placeholder="Enter coupon code"
                value={discount}
                onChange={(e) => setDiscount(e.target.value)}
              />
              <button className="mt-3 bg-success hover:bg-[#059669] text-white text-sm font-semibold px-4 py-1.5 rounded transition-colors">
                Apply Coupon
              </button>
            </div>

            {/* Price rows */}
            {[["Subtotal", `$${subtotal.toFixed(2)}`, "white"], ["Shipping", "$5.99", "white"], ["Tax", `$${(subtotal * 0.08).toFixed(2)}`, "white"], ["Discount", "-$0.00", "var(--color-success)"]].map(([label, value, color]) => (
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              <span className="text-brand text-[1.4rem] font-extrabold">${(subtotal + 5.99 + subtotal * 0.08).toFixed(2)}</span>
            </div>

            {/* Trust */}
            <p className="text-soft text-xs text-center mb-3">🔒 Secure Payment • 🚚 Fast Delivery • 💯 Quality Guarantee</p>

            {/* Payment methods */}
            <p className="text-soft text-xs text-center mb-3">Accepted Payment Methods</p>
            ...
            <button
              ...
              className="w-full bg-brand hover:bg-brand-hover disabled:bg-line-light text-white py-4 text-base font-bold rounded-lg shadow-[0_8px_25px_rgba(255,107,53,0.3)] hover:shadow-[0_12px_35px_rgba(255,107,53,0.4)] flex items-center justify-center gap-2 transition-all"
            >

...
const ProfileLayoutWrapper: React.FC<ProfileLayoutWrapperProps> = ({ children, title }) => {
  ...
  return (
    <div className="flex min-h-screen bg-surface">
      {/* Sidebar */}
      <div className="w-[280px] min-h-screen bg-white border-r border-line-light flex flex-col shadow-sm flex-shrink-0">
        {/* Profile Header */}
        <div className="p-6 border-b border-line-light">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-brand flex items-center justify-center text-white text-xl font-semibold flex-shrink-0">
              {customer?.name?.charAt(0)?.toUpperCase() || "U"}
            </div>
            <div>
              <p className="font-semibold text-heading text-[0.95rem]">{customer?.name || "User"}</p>
              <p className="text-soft text-[0.8rem]">{customer?.email || "user@example.com"}</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 py-2 px-2">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? "text-brand" : "text-soft"}`} />
                {item.label}
              </button>
            );
          })}
      ...
      <div className="flex-1 bg-surface overflow-auto">
        {/* Header */}
        <div className="bg-white border-b border-line-light px-8 py-5">

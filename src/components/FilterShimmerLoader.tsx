...
const ShimmerAccordion: React.FC<{ rows: number; defaultOpen?: boolean }> = ({ rows, defaultOpen = true }) => {
  ...
  return (
    <div className="bg-[rgba(217,217,217,0.21)] mb-1.5">
      ...
      >
        <div className="h-4 bg-wash rounded w-20 animate-pulse" />
        <ChevronDown className={`w-5 h-5 text-dim transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
        <div className="px-4 py-2 flex flex-col gap-2">
          {Array.from({ length: rows }).map((_, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className="w-4 h-4 bg-wash rounded animate-pulse flex-shrink-0" />
              <div className="h-4 bg-wash rounded animate-pulse" style={{ width: `${60 + i * 5}%` }} />
            </div>
          ))}
        </div>
      )}

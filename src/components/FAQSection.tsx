...
const FAQSection: React.FC<FAQSectionProps> = ({ faqs, isLoading, error }) => {
  ...
  if (isLoading) {
    return (
      <div className="mt-8">
        <h2 className="text-xl md:text-2xl font-semibold text-brand text-center mb-6">Frequently Asked Questions</h2>
        {[...Array(4)].map((_, i) => (
  ...
  if (!faqs || faqs.length === 0) {
    return (
      <div className="mt-8 text-center">
        <h2 className="text-xl md:text-2xl font-semibold text-brand mb-2">Frequently Asked Questions</h2>
        <p className="text-soft italic">No FAQs available for this product yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8">
      <h2
        ...
        style={{ background: "linear-gradient(135deg, var(--color-brand) 0%, var(--color-brand-hover) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}
      >
        ...
        {faqs.map((faq, index) => {
          ...
          return (
            <div
              ...
              style={{ background: isOpen ? "linear-gradient(135deg, var(--color-brand-light) 0%, var(--color-brand-light) 100%)" : "linear-gradient(135deg, #ffffff 0%, var(--color-paper) 100%)" }}
            >
              {/* Summary */}
              <button
                type="button"
              ...
              >
                <h3 className={`text-base md:text-lg font-semibold leading-snug transition-colors ${isOpen ? "text-brand" : "text-body"}`}>
                  {faq.question}
                </h3>
              ...
              {isOpen && (
                <div className="px-6 pb-6 bg-[rgba(255,107,53,0.02)] border-t border-[rgba(255,107,53,0.08)]">
                  <p className="text-dim text-[0.95rem] leading-[1.7] pt-4">{faq.answer}</p>
                </div>
              )}
            </div>
          );

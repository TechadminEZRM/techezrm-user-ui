...
const urgencyBadge = (u: string) => {
  ...
  if (u === "medium") return "bg-warn-light border-warn-light text-[#856404]";
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md p-0 rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.15)]">
        ...
        {/* Content */}
        <div className="p-6" style={{ background: "linear-gradient(180deg, var(--color-paper) 0%, #ffffff 100%)" }}>
          {/* ID Card */}
          <div className="bg-white border border-line rounded-2xl p-5 mb-5">
            {/* RFQ ID */}
            <div className="mb-4">
              <p className="text-sm font-semibold text-dim mb-2">RFQ Reference ID</p>
              <div className="flex items-center gap-2 p-3 rounded-xl border border-[#dee2e6]" style={{ background: "linear-gradient(135deg, var(--color-surface) 0%, var(--color-line) 100%)" }}>
                <span className="text-base font-bold text-success font-mono flex-1">{responseData.uniqueId}</span>
                <button
                  onClick={() => navigator.clipboard.writeText(responseData.uniqueId)}
                  className="text-soft hover:text-success hover:bg-[rgba(40,167,69,0.1)] rounded p-1 transition-colors"
                >
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <div key={label} className="flex justify-between items-center">
                  <span className="text-sm text-dim font-medium">{label}</span>
                  <span className="text-sm text-body font-semibold">{value}</span>
                </div>
              ))}

              <div className="flex justify-between items-center">
                <span className="text-sm text-dim font-medium">Status:</span>
                <span className="text-xs font-semibold uppercase px-3 py-1 rounded-full bg-warn-light border border-warn-light text-[#856404]">{responseData.status}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm text-dim font-medium">Urgency:</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-dim font-medium">Submitted:</span>
                <span className="text-sm text-body font-semibold">{formatDate(responseData.createdAt)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Next Steps */}
          <div className="mb-5">
            <p className="text-base font-semibold text-body mb-3">What happens next?</p>
            <div className="flex flex-col gap-2">
              {["Our team will review your request within 24 hours", "You'll receive a detailed quote via email", "Schedule a call to discuss your requirements"].map((step, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded-full bg-success text-white text-xs font-semibold flex items-center justify-center flex-shrink-0">{i + 1}</div>
                  <span className="text-sm text-dim">{step}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-3 justify-center">
            <button
              onClick={onClose}
              className="border border-success text-success font-semibold px-5 py-2.5 rounded-[25px] hover:bg-[rgba(40,167,69,0.1)] transition-colors"
            >
              Close
            </button>
            <button
